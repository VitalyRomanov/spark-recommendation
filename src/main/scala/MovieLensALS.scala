...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    val (ratingsPath, doGrading, user_path) = parseArgs(args)
    ...
    val myRating = if (doGrading)
        (new Grader(ratingsPath, sc, user_path)).toRDD
      else
    ...
    val (train, test, baseline, filmCount, br) = loadRatings(ratingsPath + "/ratings2.csv", sc)

//    br.take(100)
//      .foreach(x => println(s"${filmId2Title(x._1)} ${filmCount(x._1)} ${x._2}"))
//    sys.exit()
    ...
    val model = ALS.train(train.union(myRating), 100, 10)
    ...
    if (doGrading){
      ...
      model.predict(sc.parallelize(baseline.keys.map(x => (0, x)).toSeq))
        ...
        .filter(x => filmCount(x.product)>1000)
        // take first 20 items
        .take(100)
        // print title and predicted rating
        .foreach(x => println(s"${filmId2Title(x.product)}\t\t${filmCount(x.product)}\t${x.rating}"))
    }
  ...
  def parseArgs(args: Array[String]) = {
    ...
    // return tuple
    (ratingsPath, doGrading, user_path)
  }


  def baselineRating(ratings: RDD[Rating]) = {
    val average = ratings
    ...
    val average2 = ratings
      .map(x => (x.product, (x.rating * x.rating, 1:Int)))
      .reduceByKey( (r1, r2)=> (r1._1 + r2._1, r1._2 + r2._2))
      .map(x => (x._1, x._2._1 / x._2._2))

    val std = average2
      .join(average)
      .map(x => (x._1, math.sqrt(x._2._1 - x._2._2 * x._2._2)))

    val lb = average.join(std).map(x => (x._1, x._2._1 - x._2._2))

    lb.sortBy(_._2, false)
  }
  ...
  def loadRatings(path: String, sc: SparkContext) = {
    ...
    val filmCount = ratingsData.map(x=>(x.product, 1:Int)).reduceByKey(_+_).collectAsMap()

    (train, test, baseline, filmCount, baselineRating(ratingsData))
  }
  ...
  def parseTitle(filmEntry: String) = {
    ",\".*\",".r findFirstIn filmEntry match {
      case Some(s) => s.slice(2, s.length - 2)
      case None => ",.*,".r findFirstIn filmEntry match {
